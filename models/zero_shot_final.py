...
Known_Actions=["sitting","stairs"]
Unknown_Actions=["walking"]
sampling_rate = 64
...
def processEMG(file_path):
    res = []
    signal = load_data(file_path,fs=sampling_rate)
    electrodes = len(signal.v[0])


    for i in range(electrodes):
        ppg_fft = np.fft.rfft(signal.v[:][i])
        magnitude_spectrum = np.abs(ppg_fft)
        psd = np.abs(ppg_fft) ** 2
        ...
        spectral_flatness = np.exp(np.mean(np.log(magnitude_spectrum + 1e-10))) / np.mean(magnitude_spectrum)
        res += [np.mean(magnitude_spectrum),spectral_flatness, spectral_entropy] 
    return res
...
def mergePPG_EMG(ppg,emg):
    for p in ppg:
        p+=emg[random.randint(0,len(emg)-1)]
    return ppg
    


def loadPPG_EMG_Sentence(dirs,Actions):
    txt_features=[]
    ppg_emg_features=[]
    for i in range(len(Actions)):
        ...
        for s in os.listdir(os.path.join(dirs,Actions[i],"EMG")):
            # print(s,Actions[i])
            tmp_emg.append(processEMG(os.path.join(dirs,Actions[i],"EMG",s))) 
        for s in os.listdir(os.path.join(dirs,Actions[i],"PPG")):
            tmp_ppg.append(processPPG(os.path.join(dirs,Actions[i],"PPG",s)))
        tmp_txt.append(getSentenceVector(os.path.join(dirs,Actions[i],"description.txt")))
        m = mergePPG_EMG(tmp_ppg,tmp_emg)
        ppg_emg_features+=m
        txt_features+=tmp_txt*len(m)
    # print(txt_features)
    # exit()
    return txt_features, ppg_emg_features

class ContrastiveModel(nn.Module):
    def __init__(self, sentence_feature_dim, PPG_feature_dim, EMG_feature_dim, embed_dim):
        super().__init__()
        self.txt_linear = nn.Linear(sentence_feature_dim, embed_dim)
        self.ppg_emg_linear = nn.Linear(PPG_feature_dim+EMG_feature_dim, embed_dim)
        
        
    
    def forward(self, txt_feature, ppg_emg_feature):
        txt_embed = self.txt_linear(torch.tensor(txt_feature, dtype=torch.float32))
        ppg_emg_embed = self.ppg_emg_linear(torch.tensor(ppg_emg_feature, dtype=torch.float32))
        
        
        return txt_embed,ppg_emg_embed 
...
if __name__ == "__main__":
    model = ContrastiveModel(sentence_feature_dim=384,PPG_feature_dim=3, EMG_feature_dim=24, embed_dim=384)
    ...
    for epoch in range(epochs):
        ...
        for act in Known_Actions:
            txt_features, ppg_emg_features = loadPPG_EMG_Sentence("datasets",[act])
            txt_embed, ppg_emg_embed = model(txt_features, ppg_emg_features)
            Map[act] = [txt_embed, ppg_emg_embed]
    ...
    with torch.no_grad():
        ...
        # Correct Unknown
        txt_features, ppg_emg_features = loadPPG_EMG_Sentence("datasets",Unknown_Actions)
        unknown_txt_embed, unknown_ppg_emg_embed = model(txt_features, ppg_emg_features)
        similarity = cos_sim(unknown_txt_embed,unknown_ppg_emg_embed)
        print("==============================")
        ...
        for act in Map_des:
            ...
            txt_embed,_ = model(txt_features, ppg_emg_features)
            similarity = cos_sim(txt_embed,unknown_ppg_emg_embed)
        ...
        for act in Known_Actions:
            txt_features, ppg_features = loadPPG_EMG_Sentence("datasets",[act])
